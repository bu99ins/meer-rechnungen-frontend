...
React 19 / TypeScript / Vite 6 SPA for invoice management (invoices, customers, senders — CRUD + PDF download). Frontend for the **invoices-back** backend API, which lives in a separate repo/directory. Deployed to Cloudflare Pages. The repo carries no edge gate — access control is the application's own JWT login — but that is a statement about this code, not necessarily about what's live: the backend repo's `docs/deployment.md` tracks whether the deployed site has actually been cut over yet.
